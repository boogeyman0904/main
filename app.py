...
class Main:
    ...
    async def start_hend(self, msg:Message, bot:Bot):
        ...
        Bu hech narsa qilmaydigan bot. <span class='tg-spoiler'> Bu Spoiler</span>"""
    ...
    async def Menu(self, msg:Message, bot:Bot):
        men = [
            [InlineKeyboardButton(text="BOT XAQIDA",callback_data="bot xaqida"), InlineKeyboardButton(text="MUALLIF XAQIDA",callback_data="muallif xaqida")]
        ]
        rp = InlineKeyboardMarkup(inline_keyboard=men)
        await msg.answer(text="Iltimos menuni tanlang",reply_markup=rp)
    ...
    async def echo(self, msg:Message, bot:Bot):
        print(msg.dict())
        await msg.answer("Assalomu Aleykum hurmatli")
        await msg.reply("Bu replay")
